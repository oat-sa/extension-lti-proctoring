...
},stop:function(){$loadingBar.removeClass("loading fixed").height(originalHeight)}}}),define("ltiProctoring/controller/delegate",["jquery","util/url","layout/loading-bar"],function($,urlUtil,loadingBar){"use strict";var delegateCtlr={start:function(){var $container=$(".delegated-view"),delivery=$container.data("delivery"),action=$container.data("action"),controller=$container.data("controller"),extension=$container.data("extension"),page=urlUtil.route(action,controller,extension),params={delivery:delivery};$container.load(page,params,function(){loadingBar.stop()})}};return loadingBar.start(),delegateCtlr});
